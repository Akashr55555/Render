...
router.post('/', (req: Request, res: Response) => {
  uploadFiles(req, res, async (err: any) => {
    ...
    try {
      ...
      if (pwd) {
        const userPwdEsc = pwd.replace(/"/g, '\\"');
        const inputEsc = tempSavedPath.replace(/"/g, '\\"');
        const outEsc = out.replace(/"/g, '\\"');
        const cmd = `gs -sDEVICE=pdfwrite -dEncryptionR=3 -dKeyLength=128 -sUserPassword="${userPwdEsc}" -sOwnerPassword="${userPwdEsc}" -dNOPAUSE -dQUIET -dBATCH -sOutputFile="${outEsc}" "${inputEsc}"`;
