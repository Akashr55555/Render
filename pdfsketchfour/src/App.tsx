...
import { routeConfigs } from './seo/seoConfig';
import { auth, onAuthStateChanged, signOut } from './lib/firebase';
import { fetchEntitlementStatus, startCheckout, openBillingPortal } from './lib/billing';
import { 
  Heart, 
  Grid, 
  Sparkles, 
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (fbUser) => {
      ...
      setCurrentUser({
        id: fbUser.uid,
        name: fbUser.displayName || fbUser.email?.split('@')[0] || 'PDFSketch User',
        email: fbUser.email || '',
        avatar: fbUser.photoURL || undefined,
        provider: fbUser.providerData[0]?.providerId === 'password' ? 'email' :
          fbUser.providerData[0]?.providerId?.includes('google') ? 'google' :
          fbUser.providerData[0]?.providerId?.includes('microsoft') ? 'microsoft' : 'facebook',
        isPremium: false,
      });
      // isPremium above is a placeholder until the server confirms it. It is
      // never set to true from client state alone (see refreshEntitlement) -
      // only a verified Stripe webhook can make /api/billing/status say true.
      refreshEntitlement();
    });
    return () => unsubscribe();
  }, []);
  ...
  const refreshEntitlement = async () => {
    try {
      ...
      setCurrentUser(prev => prev ? { ...prev, isPremium: status.isPremium } : prev);
    } catch (e) {
      // Not signed in yet, or billing isn't configured server-side - fine,
      // the user just stays on the free plan until this succeeds.
      console.warn('[billing] could not refresh entitlement', e);
    }
  };

  // After returning from Stripe Checkout, re-check entitlement status (the
  // webhook that actually grants premium runs async, slightly before or
  // after this redirect lands) and clean the query string.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const checkout = params.get('checkout');
    if (checkout === 'success') {
      showToast('Payment received! Activating your premium plan…');
      refreshEntitlement();
      setTimeout(refreshEntitlement, 3000); // webhook may land a moment later
  ...
  const handleSelectPlan = async (plan: 'monthly' | 'annual') => {
    // This never sets isPremium directly - it only redirects to Stripe
    // Checkout. Premium is granted exclusively by the server after Stripe
    // confirms payment (see server/routes/billingWebhook.ts), then picked
    // up here via refreshEntitlement() on the '?checkout=success' return.
    if (!currentUser) {
      setAuthMode('login');
      setAuthModalOpen(true);
      showToast('Please sign in first to subscribe to Premium.');
      return;
    }
    ...
    try {
      ...
      showToast(e.message || 'Could not start checkout. Please try again.');
    } finally {
  ...
  const popularToolIds = ['merge', 'compress', 'pdftoword', 'split', 'edit'];
  const popularTools = popularToolIds.map(id => tools.find(t => t.id === id)).filter(Boolean) as Tool[];
    ...
    <Router>
          ...
          <header className="fixed top-0 left-0 right-0 h-16 bg-white/90 backdrop-blur-md border-b border-slate-200/80 z-40 px-4 md:px-8 flex items-center justify-between shadow-xs">
            <div className="flex items-center gap-8">
              ...
              <nav className="hidden lg:flex items-center gap-6 text-xs font-bold tracking-wider uppercase text-slate-600">
                ...
                <button 
                  onClick={() => { navigate('/'); setSelectedFilter('all'); setSearchQuery(''); }} 
                  className={`hover:text-indigo-600 transition-colors ${selectedFilter === 'all' && isHome ? 'text-indigo-600 font-extrabold' : ''}`}
                >
                  {t('allTools')}
                </button>
              </nav>
                ...
                <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 text-xs">
                  ...
                  {currentUser.isPremium ? (
                    <button
                      ...
                      className="w-full text-left px-3.5 py-2 text-slate-700 hover:bg-slate-50 font-bold flex items-center gap-2"
                    >
                      <Crown className="w-3.5 h-3.5 text-amber-500 fill-amber-400" /> Manage billing
                    </button>
                  ) : (
                    <button
                      onClick={() => {
      ...
      <main className="mt-14 flex-1">
        ...
        <section className="max-w-6xl mx-auto text-center px-4 pt-6 sm:pt-10 pb-6">
          ...
          >
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-[44px] font-black font-heading tracking-tight leading-tight mb-3.5">
            ...
            <p className="text-slate-600 text-xs sm:text-sm md:text-base lg:text-[17px] max-w-3xl mx-auto font-normal leading-relaxed mb-4">
              {t('heroSubtitle')}
            </p>
        ...
        {/* Popular tools — shortest path to high-intent actions */}
        <section className="max-w-7xl mx-auto px-4 mb-6" aria-labelledby="popular-tools-title">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 rounded-2xl border border-slate-200 bg-white/80 p-3 shadow-sm">
            <span id="popular-tools-title" className="shrink-0 px-2 text-[11px] font-black uppercase tracking-[0.14em] text-slate-500">Popular</span>
            <div className="flex flex-wrap gap-2">
              {popularTools.map((tool) => {
                const config = Object.values(routeConfigs).find(rc => rc.toolId === tool.id);
                return (
                  <button
                    key={tool.id}
                    type="button"
                    onClick={() => config ? navigate(config.path) : setActiveTool(tool)}
                    className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-700 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 transition-colors"
                  >
                    <ToolIcon name={tool.icon} />
                    {getToolTranslation(tool, currentLanguage.code).title}
                  </button>
                );
              })}
            </div>
          ...
          >
            <AnimatePresence>
              {filteredTools.map(tool => {
                ...
                const matchConfig = Object.values(routeConfigs).find(rc => rc.toolId === tool.id);
                const toolPath = matchConfig ? matchConfig.path : '#';
                return (
                  <motion.a
                    key={tool.id}
                    href={toolPath}
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      if (matchConfig) {
                        navigate(matchConfig.path);
                      } else {
                        setActiveTool(tool);
                      }
                    }}
                    className="glow-card bg-white border border-slate-200/90 hover:border-indigo-300 rounded-2xl p-5 flex flex-col justify-between cursor-pointer transition-all relative group min-h-[190px] text-left no-underline"
                  >
                    {/* Top Badges */}
      ...
      <footer className="bg-slate-900 text-slate-400 text-xs py-12 border-t border-slate-800">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-6 gap-8 mb-8">
          ...
          <div>
            <h4 className="text-white font-bold uppercase tracking-wider mb-3">Convert PDF</h4>
            <ul className="space-y-2">
              <li><a href="/pdf-to-word/" onClick={(e) => { e.preventDefault(); navigate('/pdf-to-word/'); }} className="hover:text-purple-400 transition-colors">PDF to Word</a></li>
              <li><a href="/pdf-to-excel/" onClick={(e) => { e.preventDefault(); navigate('/pdf-to-excel/'); }} className="hover:text-purple-400 transition-colors">PDF to Excel</a></li>
              <li><a href="/pdf-to-ppt/" onClick={(e) => { e.preventDefault(); navigate('/pdf-to-ppt/'); }} className="hover:text-purple-400 transition-colors">PDF to PowerPoint</a></li>
              <li><a href="/pdf-to-jpg/" onClick={(e) => { e.preventDefault(); navigate('/pdf-to-jpg/'); }} className="hover:text-purple-400 transition-colors">PDF to JPG</a></li>
              <li><a href="/word-to-pdf/" onClick={(e) => { e.preventDefault(); navigate('/word-to-pdf/'); }} className="hover:text-purple-400 transition-colors">Word to PDF</a></li>
              <li><a href="/excel-to-pdf/" onClick={(e) => { e.preventDefault(); navigate('/excel-to-pdf/'); }} className="hover:text-purple-400 transition-colors">Excel to PDF</a></li>
              <li><a href="/jpg-to-pdf/" onClick={(e) => { e.preventDefault(); navigate('/jpg-to-pdf/'); }} className="hover:text-purple-400 transition-colors">JPG to PDF</a></li>
              <li><a href="/html-to-pdf/" onClick={(e) => { e.preventDefault(); navigate('/html-to-pdf/'); }} className="hover:text-purple-400 transition-colors">HTML to PDF</a></li>
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold uppercase tracking-wider mb-3">Organize & Edit</h4>
            <ul className="space-y-2">
              <li><a href="/merge-pdf/" onClick={(e) => { e.preventDefault(); navigate('/merge-pdf/'); }} className="hover:text-purple-400 transition-colors">Merge PDF</a></li>
              <li><a href="/split-pdf/" onClick={(e) => { e.preventDefault(); navigate('/split-pdf/'); }} className="hover:text-purple-400 transition-colors">Split PDF</a></li>
              <li><a href="/compress-pdf/" onClick={(e) => { e.preventDefault(); navigate('/compress-pdf/'); }} className="hover:text-purple-400 transition-colors">Compress PDF</a></li>
              <li><a href="/organize-pdf/" onClick={(e) => { e.preventDefault(); navigate('/organize-pdf/'); }} className="hover:text-purple-400 transition-colors">Organize PDF</a></li>
              <li><a href="/edit-pdf/" onClick={(e) => { e.preventDefault(); navigate('/edit-pdf/'); }} className="hover:text-purple-400 transition-colors">Edit PDF</a></li>
              <li><a href="/rotate-pdf/" onClick={(e) => { e.preventDefault(); navigate('/rotate-pdf/'); }} className="hover:text-purple-400 transition-colors">Rotate PDF</a></li>
              <li><a href="/crop-pdf/" onClick={(e) => { e.preventDefault(); navigate('/crop-pdf/'); }} className="hover:text-purple-400 transition-colors">Crop PDF</a></li>
              <li><a href="/page-numbers/" onClick={(e) => { e.preventDefault(); navigate('/page-numbers/'); }} className="hover:text-purple-400 transition-colors">Add Page Numbers</a></li>
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold uppercase tracking-wider mb-3">Security & Signing</h4>
            <ul className="space-y-2">
              <li><a href="/protect-pdf/" onClick={(e) => { e.preventDefault(); navigate('/protect-pdf/'); }} className="hover:text-purple-400 transition-colors">Protect PDF</a></li>
              <li><a href="/unlock-pdf/" onClick={(e) => { e.preventDefault(); navigate('/unlock-pdf/'); }} className="hover:text-purple-400 transition-colors">Unlock PDF</a></li>
              <li><a href="/watermark-pdf/" onClick={(e) => { e.preventDefault(); navigate('/watermark-pdf/'); }} className="hover:text-purple-400 transition-colors">Watermark PDF</a></li>
              <li><a href="/sign-pdf/" onClick={(e) => { e.preventDefault(); navigate('/sign-pdf/'); }} className="hover:text-purple-400 transition-colors">Sign PDF</a></li>
              <li><a href="/redact-pdf/" onClick={(e) => { e.preventDefault(); navigate('/redact-pdf/'); }} className="hover:text-purple-400 transition-colors">Redact PDF</a></li>
              <li><a href="/ocr-pdf/" onClick={(e) => { e.preventDefault(); navigate('/ocr-pdf/'); }} className="hover:text-purple-400 transition-colors">OCR PDF</a></li>
              <li><a href="/repair-pdf/" onClick={(e) => { e.preventDefault(); navigate('/repair-pdf/'); }} className="hover:text-purple-400 transition-colors">Repair PDF</a></li>
              <li><a href="/pdf-to-pdfa/" onClick={(e) => { e.preventDefault(); navigate('/pdf-to-pdfa/'); }} className="hover:text-purple-400 transition-colors">PDF to PDF/A</a></li>
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold uppercase tracking-wider mb-3">Resources & More</h4>
            <ul className="space-y-2">
              <li><a href="/guides/" onClick={(e) => { e.preventDefault(); navigate('/guides/'); }} className="hover:text-purple-400 transition-colors">Knowledge Hub & Guides</a></li>
              <li><a href="/compare-pdf/" onClick={(e) => { e.preventDefault(); navigate('/compare-pdf/'); }} className="hover:text-purple-400 transition-colors">Compare PDF</a></li>
              <li><a href="/pdf-forms/" onClick={(e) => { e.preventDefault(); navigate('/pdf-forms/'); }} className="hover:text-purple-400 transition-colors">PDF Forms</a></li>
              <li><a href="/scan-pdf/" onClick={(e) => { e.preventDefault(); navigate('/scan-pdf/'); }} className="hover:text-purple-400 transition-colors">Scan to PDF</a></li>
              <li><a href="/translate-pdf/" onClick={(e) => { e.preventDefault(); navigate('/translate-pdf/'); }} className="hover:text-purple-400 transition-colors">Translate PDF</a></li>
              <li><a href="/pdf-to-markdown/" onClick={(e) => { e.preventDefault(); navigate('/pdf-to-markdown/'); }} className="hover:text-purple-400 transition-colors">PDF to Markdown</a></li>
              <li><a href="/ai-summarize/" onClick={(e) => { e.preventDefault(); navigate('/ai-summarize/'); }} className="hover:text-purple-400 transition-colors">AI PDF Summarizer</a></li>
              <li><a href="/workflow-custom/" onClick={(e) => { e.preventDefault(); navigate('/workflow-custom/'); }} className="hover:text-purple-400 transition-colors">Custom Workflows</a></li>
              <li><a href="/pricing/" onClick={(e) => { e.preventDefault(); navigate('/pricing/'); }} className="hover:text-purple-400 transition-colors">Pricing</a></li>
              <li><a href="/business/" onClick={(e) => { e.preventDefault(); navigate('/business/'); }} className="hover:text-purple-400 transition-colors">Business</a></li>
              <li><a href="/teams/" onClick={(e) => { e.preventDefault(); navigate('/teams/'); }} className="hover:text-purple-400 transition-colors">Teams</a></li>
              <li><a href="/education/" onClick={(e) => { e.preventDefault(); navigate('/education/'); }} className="hover:text-purple-400 transition-colors">Education</a></li>
              <li><a href="/security/" onClick={(e) => { e.preventDefault(); navigate('/security/'); }} className="hover:text-purple-400 transition-colors">Security</a></li>
            </ul>
          </div>
        </div>

        <div className="max-w-7xl mx-auto px-4 border-t border-slate-800 pt-6 pb-6 mb-2">
          <h4 className="text-white font-bold uppercase tracking-wider mb-3">PDF Guides & Tutorials</h4>
          <ul className="flex flex-wrap gap-x-6 gap-y-2">
            {Object.values(routeConfigs).filter(rc => rc.isGuide && rc.path !== '/guides/').map(guide => (
              <li key={guide.path}>
                <a href={guide.path} onClick={(e) => { e.preventDefault(); navigate(guide.path); }} className="hover:text-purple-400 transition-colors">
                  {guide.h1}
                </a>
              </li>
            ))}
