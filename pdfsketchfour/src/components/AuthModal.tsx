...
import { X, Mail, Lock, User as UserIcon, Key, Check, AlertCircle } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSocialLogin = (provider: 'google' | 'facebook' | 'sso') => {
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      const user: User = {
        id: 'usr_' + provider + '_' + Date.now(),
        name: provider === 'google' ? 'Google User' : provider === 'facebook' ? 'Facebook User' : 'SSO Member',
        email: `user@${provider}.com`,
        provider: provider,
        isPremium: provider === 'sso',
      };
      onLoginSuccess(user);
      onClose();
    }, 700);
