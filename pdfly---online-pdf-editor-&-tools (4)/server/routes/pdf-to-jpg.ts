...
router.post('/', (req: Request, res: Response) => {
  up(req, res, async (err: any) => {
    ...
    try {
      ...
      const zipPath = outPath('pdf2jpg', 'zip');
      const stream = fs.createWriteStream(zipPath);
      const arc = new archiver.ZipArchive({ zlib: { level: 6 } });
      stream.on('close', () => {
        if (req.file) cleanup([req.file.path]);
        res.setHeader('Content-Type', 'application/zip');
        res.setHeader('Content-Disposition', 'attachment; filename="pages-jpg.zip"');
      ...
      for (let i = 1; i <= doc.numPages; i++) {
        ...
        arc.append(jpg, { name: `page-${String(i).padStart(3, '0')}.jpg` });
      }
      arc.finalize();
    } catch (e: any) {
      if (req.file) cleanup([req.file.path]);
